...
const combinacoesDir = () => path.join(process.cwd(), 'content', 'combinacoes')

export function getCombinacao(slug) {
  const p = path.join(combinacoesDir(), `${slug}.json`)
  if (!/^[a-z]+-e-[a-z]+$/.test(String(slug || '')) || !fs.existsSync(p)) return null
  return JSON.parse(fs.readFileSync(p, 'utf8'))
}

let combinacoesCache = null
export function getTodasCombinacoes() {
  if (combinacoesCache) return combinacoesCache
  const dir = combinacoesDir()
  if (!fs.existsSync(dir)) return []
  combinacoesCache = fs
    .readdirSync(dir)
    .filter((f) => f.endsWith('.json'))
    .map((f) => JSON.parse(fs.readFileSync(path.join(dir, f), 'utf8')))
  return combinacoesCache
}
