...
export function emailDigest({ dataFmt, cidades, noticias, email }) {
    ...
    .map(
      ...
      <tr><td style="padding:11px 0;border-bottom:1px solid #eee">
        <table cellpadding="0" cellspacing="0"><tr>
          <td style="vertical-align:top;padding-right:10px"><span style="font-size:15px;font-weight:900;color:#FF6B00">${i + 1}</span></td>
          <td><a href="${SITE_URL}/noticia/${a.slug}" style="color:#131417;text-decoration:none;font-weight:600;font-size:14.5px;line-height:1.4">${a.title}</a>${a.category ? `<div style="font-size:11px;color:#8a8f98;text-transform:uppercase;letter-spacing:.5px;margin-top:3px">${a.category}</div>` : ''}</td>
        </tr></table>
      </td></tr>`
    )
    .join('')
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f0"><tr><td align="center" style="padding:24px 12px">
    <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif">
      <tr><td style="background:#0C0E1A;border-radius:16px 16px 0 0;padding:28px 30px;text-align:center">
        ...
        <div style="font-size:11px;letter-spacing:2px;color:rgba(255,255,255,.5);text-transform:uppercase;margin-top:6px">Seu resumo diário</div>
      </td></tr>
      <tr><td style="background:#ffffff;padding:30px 30px 24px">
        <div style="font-size:22px;font-weight:900;color:#131417;letter-spacing:-.4px">Bom dia! ☀️</div>
        ...
        ${noticias && noticias.length ? `<div style="font-size:12px;letter-spacing:1.2px;text-transform:uppercase;color:#8a8f98;font-weight:700;margin-bottom:4px">📰 As últimas notícias</div><table width="100%" cellpadding="0" cellspacing="0">${linhasNews}</table>` : ''}
  ...
  return {
    assunto: `☀️ Bom dia! Seu resumo — ${dataFmt.split(',')[0]}`,
    html,
