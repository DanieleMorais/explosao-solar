...
// Resumo diário: bom dia + clima + últimas notícias.
export function emailDigest({ dataFmt, clima, noticias, email }) {
  const linhasClima = (clima || [])
    .map((c) => `<tr><td style="padding:4px 0;font-size:14px">${c.emoji} <strong>${c.nome}</strong></td><td style="padding:4px 0;font-size:14px;text-align:right;color:#3d4149">${c.temp}° <span style="color:#8a8f98">(${c.min}°–${c.max}°)</span>${c.chuva >= 50 ? ` · 💧${c.chuva}%` : ''}</td></tr>`)
    .join('')
  const linhasNews = (noticias || [])
    .map(
      (a, i) =>
        `<tr><td style="padding:9px 0;border-bottom:1px solid #eee"><a href="${SITE_URL}/noticia/${a.slug}" style="color:#131417;text-decoration:none;font-weight:600;font-size:14.5px;line-height:1.4">${i + 1}. ${a.title}</a></td></tr>`
    )
    .join('')

  const conteudo = `
    <h1 style="font-size:20px;margin:0 0 4px">Bom dia! ☀️</h1>
    <p style="margin:0 0 20px;color:#8a8f98;font-size:13px;text-transform:capitalize">${dataFmt}</p>
    ${clima && clima.length ? `<h2 style="font-size:13px;letter-spacing:1px;text-transform:uppercase;color:#8a8f98;margin:0 0 8px">🌤️ O tempo hoje</h2><table style="width:100%;border-collapse:collapse;margin-bottom:6px">${linhasClima}</table><p style="margin:0 0 22px;font-size:12.5px"><a href="${SITE_URL}/clima/brasil" style="color:#FF6B00">Ver o clima do seu bairro →</a></p>` : ''}
    ${noticias && noticias.length ? `<h2 style="font-size:13px;letter-spacing:1px;text-transform:uppercase;color:#8a8f98;margin:0 0 6px">📰 As últimas notícias</h2><table style="width:100%;border-collapse:collapse">${linhasNews}</table>` : ''}
    <p style="margin:24px 0 0"><a href="${SITE_URL}" style="display:inline-block;background:linear-gradient(100deg,#FF6B00,#FFB300);color:#131417;font-weight:800;text-decoration:none;padding:12px 24px;border-radius:999px">Ler o portal →</a></p>`

  return {
    assunto: `☀️ Explosão Solar — seu resumo de ${dataFmt.split(',')[0]}`,
    html: WRAP(conteudo, email),
    texto: `Bom dia! ${dataFmt}\n\n${(clima || []).map((c) => `${c.nome}: ${c.temp}° (${c.min}-${c.max})`).join('\n')}\n\nÚltimas: ${(noticias || []).map((a) => a.title).join(' | ')}\n\n${SITE_URL}`,
  }
}
