...
// UA de navegador: Open-Meteo (como a guidi e a AwesomeAPI) recusa/limita requisição
// sem User-Agent vinda de IP de datacenter (Worker da CF e runner do Actions vão sem UA).
const UA_NAVEGADOR = {
...
async function jsonComTimeout(url, ms = 12000) {
  ...
  for (let tentativa = 0; tentativa < 2; tentativa++) {
    try {
      // no-store: sem isso, a entrada do cache de dados assada no BUILD (somente-leitura
      // na Cloudflare) seria servida pra sempre e o "agora" ficaria defasado (garoa fantasma).
      const r = await fetch(url, { headers: UA_NAVEGADOR, signal: AbortSignal.timeout(ms), cache: 'no-store' })
...
export async function previsaoPonto(lat, lon) {
  try {
    const d = await jsonComTimeout(
      `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,weather_code,relative_humidity_2m,wind_speed_10m,apparent_temperature,is_day&hourly=temperature_2m,weather_code,precipitation_probability,is_day&daily=temperature_2m_max,temperature_2m_min,weather_code,precipitation_probability_max&timezone=auto&forecast_days=14`
    )
...
export async function climaAnual(lat, lon) {
  try {
    const j = await fetch(
      ...
      { headers: UA_NAVEGADOR, signal: AbortSignal.timeout(20000), next: { revalidate: 2592000 } }
    ).then((r) => r.json())
